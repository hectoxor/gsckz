...
class Review extends AppModel{
	...
	public function customUploadImg($file = array()){
		...
		if($file['img']['type'] == 'image/jpeg' || $file['img']['type'] == 'image/png'){
		...
		$path = WWW_ROOT . 'img/reviews/' . $fileName;
		$path_th = WWW_ROOT . 'img/reviews/thumbs/' . $fileName;
		if(!move_uploaded_file($file['img']['tmp_name'], $path)){
			return false;
		}
		$this->resizeImg($path, $path_th, 120, 120, $ext);
	...
	public function genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img/reviews/' . $name)){
