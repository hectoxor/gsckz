...
class LanguageProgram extends AppModel{
	public $actsAs = array(
		'Translate' => array(
			'title',
			'breadcrumbs',
	...
	// 	'CategoryVuz' => array(
 //            'className'  => 'CategoryVuz',
 //            // 'conditions' => array('Comment.type' => 'News', 'Comment.active' => 1),
 //            'foreignKey' => 'category_project_id',
 //            // 'counterCache' => true
 ...
 //        ),
	...
	public function customUploadImg($file = array()){
		...
		$path = WWW_ROOT . 'img/language_programs/' . $fileName;
		$path_th = WWW_ROOT . 'img/language_programs/thumbs/' . $fileName;
		if(!move_uploaded_file($file['img']['tmp_name'], $path)){
			return false;
		}
		$this->resizeImg($path, $path_th, 606, 371, $ext);
	...
	public function genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img/language_programs/' . $name)){
